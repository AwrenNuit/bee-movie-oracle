...
const deepWisdomFromTheSourceOfAllKnowledge = document.getElementById('oracle-vessel');

function generateAndRenderOracleProphecy() {
  const oracleProphecy = generateOracleProphecy();
  renderOracleProphecy(oracleProphecy);
}

function generateOracleProphecy() {
  const numberOfLines = Math.floor(Math.random() * 3) + 1;
  const startingLine = Math.floor(Math.random() * beeMovieScript.length);
  let oracleProphecy = [];
  for (let i = 0; i < numberOfLines; i++) {
    oracleProphecy.push(beeMovieScript[startingLine + i]);
  }
  return oracleProphecy;
}

function renderOracleProphecy(oracleProphecy) {
  deepWisdomFromTheSourceOfAllKnowledge.innerHTML = '';
  for (const unspeakableWisdom of oracleProphecy) {
    const $p = document.createElement('p');
    $p.classList.add('oracle-prophecy');
    $p.innerHTML = unspeakableWisdom;
    deepWisdomFromTheSourceOfAllKnowledge.append($p);
  }
}

generateAndRenderOracleProphecy();
